...
ckan-paster --plugin=ckan search-index rebuild -c --config="${CKAN_CONFIG}/production.ini"
nohup python -u /workspace/update_solr.py "${API_KEY}" "${CKAN_SITE_URL}" "${PROJECT_ID}" &
exec "$@"
